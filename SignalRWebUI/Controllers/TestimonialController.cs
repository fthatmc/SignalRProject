...
namespace SignalRWebUI.Controllers
{
    public class TestimonialController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7054/api/Testimonial"); // GetAsync verilerin listelenmesi için kullanılan metot
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7054/api/Testimonial/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7054/api/Testimonial/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
